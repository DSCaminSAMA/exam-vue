<template>
  <div class="app-container">
    <el-form :model="form" ref="form" label-width="100px" v-loading="formLoading" :rules="rules">
      ...
      <el-form-item label="课程名称：" prop="subjectId" required>
        <el-select v-model="form.subjectId" placeholder="学科" >
          <el-option v-for="item in subjectFilter" :key="item.id" :value="item.id" :label="item.name+' ( '+item.levelName+' )'"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="题干：" v-model="form.title"  @click.native="inputClick(form,'title')" prop="title" required>
        <el-input  v-html="form.title"/>
      </el-form-item>
      <el-form-item label="选项：" required>
        <el-form-item :label="item.prefix"  v-model="item.prefix"  :key="item.prefix"  v-for="(item,index) in form.items"  label-width="50px" class="question-item-label">
          <!--el-input v-html="item.prefix"  v-model="item.prefix" style="width:50px;" />-->
          <div @click="inputClick(item,'content')">
          <el-input v-html="item.content"  v-model="item.content"   class="question-item-content-input"/>
           <el-button type="danger" size="mini" class="question-item-remove" icon="el-icon-delete" @click="questionItemRemove(index)"></el-button>
          </div>
        </el-form-item>
      </el-form-item>
      <el-form-item label="解析：" v-model="form.analyze" @click.native="inputClick(form,'analyze')" prop="analyze" required>
        <el-input v-html="form.analyze"/>
      </el-form-item>
      ...
      <el-form-item label="知识点：" prop="knowledgePoint" required>
